...
const OrderDetails = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <h1 className="text-4xl font-serif text-gray-900 mb-10 text-center">{t('order.title', { id: order._id })}</h1>

      <div className="max-w-5xl mx-auto card-strong p-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          {/* Order Details Column */}
          <div className="lg:col-span-2 space-y-8">
            <div>
              <h2 className="text-2xl font-serif text-gray-800 mb-4 flex items-center"><MapPin className="mr-3" /> {t('order.shipping_title')}</h2>
              <div className="card p-5">
                <p className="text-gray-700"><strong>{t('order.name_label')}:</strong> {order.user.name}</p>
                <p className="text-gray-700"><strong>{t('order.email_label')}:</strong> <a href={`mailto:${order.user.email}`} className="text-primary hover:underline">{order.user.email}</a></p>
            ...
            <div>
              <h2 className="text-2xl font-serif text-gray-800 mb-4 flex items-center"><CreditCard className="mr-3" /> {t('order.payment_title')}</h2>
              <div className="card p-5">
                <p className="text-gray-700"><strong>{t('order.method_label')}:</strong> {order.paymentMethod}</p>
                {order.isPaid ? (
                  <div className="mt-3 text-green-600 flex items-center">
            ...
            <div>
              <h2 className="text-2xl font-serif text-gray-800 mb-4 flex items-center"><Package className="mr-3" /> {t('order.items_title')}</h2>
              <div className="card p-5 space-y-4">
                {order.orderItems.map((item) => (
                  <div key={item.product} className="flex items-center justify-between border-b border-gray-100 pb-4 last:border-b-0 last:pb-0">
                    <div className="flex items-center">
                      <div className="w-16 h-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-md flex items-center justify-center shadow-inner mr-4">
                        <img 
                      ...
                      <div>
                        <Link to={`/product/${item.product}`} className="text-lg font-medium text-gray-800 hover:text-primary hover:underline">
                          {item.name}
                        </Link>
                        <p className="text-gray-600 text-sm">{item.qty} x AED {(item.price * 3.67).toFixed(2)}</p>
                      </div>
                    </div>
                    <span className="text-lg font-bold text-gray-900">AED {(item.qty * item.price * 3.67).toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="lg:col-span-1">
            <div className="card p-6 sticky top-24">
              <h2 className="text-xl font-serif font-bold text-gray-900 mb-6">{t('order.summary_title')}</h2>
              ...
              <div className="mt-8 pt-8 border-t border-gray-100">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold text-gray-900">{t('order.ticket_title')}</h3>
                </div>
                
                <div className="bg-gray-50 rounded-2xl p-6 text-center">
                  <div className="bg-white p-3 rounded-xl shadow-sm inline-block mb-4 border border-gray-100 min-w-[140px] min-h-[140px]">
                    {qrCodeUrl ? (
                  ...
                  <p className="text-xs text-gray-500 mb-4 leading-relaxed px-2">
                    {t('order.ticket_scan_hint')}
                  </p>
                  <button 
                    onClick={downloadQR}
                    className="flex items-center justify-center gap-2 w-full py-3 bg-white border border-gray-200 text-gray-700 rounded-xl text-sm font-bold hover:bg-gray-50 transition-colors shadow-sm"
                  >
